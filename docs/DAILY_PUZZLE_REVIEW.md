...
1. **5 of the 19 seeded clubs are dead ends.** Borussia Dortmund, Inter Milan, Ajax, AS Roma, and Bayer Leverkusen exist in the `clubs` table but have zero player edges (`scripts/seed-dev.ts`). They still appear in the autocomplete (`/api/catalog` has no connectivity filter), so picking one always fails validation with no signal that it was ever winnable. (Deferred — the club dataset isn't complete yet and will later be built from an open-source player database.)
